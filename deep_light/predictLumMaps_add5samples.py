...
from deep_light.pano2fish_lum import save_im_falsecolor, plotDGPAnalysis_whole, plot_DGPvsSC_Analysis, plotLog10MSEAnalysis, plotLog10RERAnalysis, plotLog10MSEAnalysis01, plotLog10RERAnalysis01
...
# function to display the comparison between ground truth, predicted and difference images on 1 image
def saveComparisonImage(error, i, n_im, p_im, diff, type, mse, dir, dif, al, az, sc_p, sc_t, sc_diff,
                        solid_angle_mse, solid_angle_rer, gradient, vmax):
    # setup the figure
    fig = plt.figure(figsize = (10, 10))
    plt.suptitle(type + " Image Comparison: %.d, Altitude %d, Azimuth %d, Direct Irradiance %.d, Diffuse Irradiance %d,"
                 % (i, al, az, dir, dif))
    plt.figtext(.5, .93, " Average Error Rate: %.4f, MSE: %.5e, Solid Angle Weighted MSE: %.5e, Solid Angle Weighted RER: %.4f"
                % (error, mse, solid_angle_mse, solid_angle_rer), fontsize=10, ha='center')
    plt.figtext(.5, .91, " Spatial Contrast(SC) Error: %.5e, Predicted SC: %.5e, Truth SC: %.5e, Gradient(sobel) MAE: %.5e"
    ...
    # save the image
    plt.savefig(os.path.join("results/result_combo/" + str(al) + "_" + str(az) + "_" +
                             str(dir) + "_" + str(dif) + "_" + type + "_" + str(vmax) + ".png"))
    plt.close()

def save_sc_ims(sc_p, sc_t, al, az, dir, dif, solid_angle_rer, solid_angle_mse, vmax):
    ...
    # setup the figure
    fig = plt.figure(figsize = (10, 10))
    plt.suptitle("Image Comparison: Altitude %d, Azimuth %d, Direct Irradiance %.d, Diffuse Irradiance %d,"
                 % (al, az, dir, dif))
    ...
    # save the image
    plt.savefig(os.path.join("results/result_combo_SC/" + str(al) + "_" + str(az) + "_" +
                             str(dir) + "_" + str(dif)  + "_" + str(vmax) + ".png"))
    plt.close()

# save gradient images
def save_gradient_ims(sobel_p, sobel_n, solid_angle_mse, solid_angle_rer, al, az, dir, dif, vmax):
    ...
    # save the image
    plt.savefig(os.path.join("results/result_combo_Sobel/" + str(al) + "_" + str(az) + "_" +
                             str(dir) + "_" + str(dif)  + "_" + str(vmax) + ".png"))
    plt.close()

# function to display the comparison between ground truth, predicted and difference images seperately
def saveComparisonImageSeperate(i, n_im, p_im, diff, type, dir, dif, al, az):
    ...
    np.savetxt(os.path.join("results/result_seperate/" +  str(al) + "_" + str(az)+ "_"+ str(dir) + "_" + str(dif) +"_" +"prediction_"  + ".txt"), p_im_2)
    np.savetxt(os.path.join("results/result_seperate/" +  str(al) + "_" + str(az)+ "_"+ str(dir) + "_" + str(dif) +"_" +"prediction_"  + ".txt"), n_im_2)
...
def testGenerator_Dense(input_dim, x_full, x_sun, batch_size):
    ...
    while 1:
        for i in range(steps_per_epoch):
            x_room_b = x_full[i, :, :, :].reshape(batch_size_pixels, input_dim-1)
...
def loadData(input_dim, output_dim):
    ...
    train_unwrapped = train.reshape(train.shape[0], IMAGE_SIZE[0], IMAGE_SIZE[1], input_dim + output_dim)
    x_total = train_unwrapped[..., :input_dim -1]
    x_sun_total = train_unwrapped[..., -2]
    x_sun_total = x_sun_total.reshape(train.shape[0], IMAGE_SIZE[0], IMAGE_SIZE[1], 1)
    y_total = train_unwrapped[..., -1]
    y_total =y_total.reshape(train.shape[0], IMAGE_SIZE[0], IMAGE_SIZE[1], 1)


    val_unwrapped = val.reshape(val.shape[0], IMAGE_SIZE[0], IMAGE_SIZE[1], input_dim + output_dim)
    x_val = val_unwrapped[..., :input_dim - 1]
    x_sun_val = val_unwrapped[..., -2]
    x_sun_val = x_sun_val.reshape(val.shape[0], IMAGE_SIZE[0], IMAGE_SIZE[1], 1)
    y_val = val_unwrapped[..., -1]
    y_val = y_val.reshape(val.shape[0], IMAGE_SIZE[0], IMAGE_SIZE[1], 1)


    test_unwrapped = test.reshape(test.shape[0], IMAGE_SIZE[0], IMAGE_SIZE[1], input_dim + output_dim)
    x_test = test_unwrapped[..., :input_dim - 1]
    x_sun_test = test_unwrapped[..., -2]
    x_sun_test = x_sun_test.reshape(test.shape[0], IMAGE_SIZE[0], IMAGE_SIZE[1], 1)
...
def get_spatial_contrast_MAE(y_true, y_pred):
    return np.mean(abs(spatial_contrast_eval(y_true) - spatial_contrast_eval(y_pred)))

def get_spatial_contrast_RER(y_true, y_pred):
    return get_relative_error(spatial_contrast_eval(y_pred), spatial_contrast_eval(y_true))
...
def mse_rer_loss_old(y_true, y_pred):
    mse_lum = K.mean(K.square(y_pred - y_true), axis=-1)
    return mse_lum + relative_err(y_true, y_pred)* LOSS_VAR + 10 * mse_log(y_true, y_pred)
...
def solid_angle_mse_rer_and_gradient_loss(y_true, y_pred):
    return 0.1*mse_loss(y_true*im_angle(), y_pred*im_angle()) + 0.8*relative_err(y_true*im_angle(), y_pred*im_angle()) \
           + 10*sobelLoss(y_true*im_angle(), y_pred*im_angle())
...
def solid_angle_mse_rer_and_spatial_contrast_loss(y_true, y_pred):
    return 0.8 * solid_anlge_mse_rer_loss(y_true, y_pred) + 1 * spatial_contrast_mae_loss(y_true, y_pred);
...
def plotLoss(history):
    ...
    plt.ylabel('val_loss')
    plt.xlabel('epoch')
    plt.legend(['test accuracy_mse'], loc='upper left')
    plt.xlim([0, NUM_EPOCHS])
    plt.ylim([0, history.history['loss'][0]])
    plt.savefig(os.path.join("validation mse.png"))
    plt.close()

def np2txt(array, al, az, dir, dif):
    array = array/179
    print(array.shape)
    np.savetxt("results/final_predictions_txt/"+ str(al) + "_" + str(az) + "_" + str(dir) + "_" + str(dif) + '_three_columns.txt', np.c_[array, array, array])
...
def getMaxMinPrams(type):
    file = open("data/processed_data/" + type +'restoration_min_max_parameters.txt', 'r')
    data = file.read().split(',')
...
def plotAnalysis(output_dim, log_process, y_predict, y, x, type, gamma, log_base):
    num = int(x.shape[0])
    print(x.shape)
    ...
    for i in range(num):
        ...
        AL_MIN, AL_MAX, AZ_MIN, AZ_MAX, DIR_MIN, DIR_MAX, DIF_MIN, DIF_MAX = getMaxMinPrams(type)
        ...
        print("direct irradiance and diffuse irradiance is: " + str(dir) + ", "  + str(dif))
        ...
        solid_anlge_weighted_mse  = ((sa_weighted_n_im-sa_weighted_p_im) ** 2).mean()
        ...
        save_sc_ims(sc_p, sc_t, al, az, dir, dif, solid_angle_weighted_error, solid_anlge_weighted_mse, 3000)
        save_sc_ims(sc_p, sc_t, al, az, dir, dif, solid_angle_weighted_error, solid_anlge_weighted_mse, 10000)
        ...
        #save gradient images
        save_gradient_ims(sobel_p, sobel_n, solid_anlge_weighted_mse, solid_angle_weighted_error, al, az, dir, dif, 3000)
        save_gradient_ims(sobel_p, sobel_n, solid_anlge_weighted_mse, solid_angle_weighted_error, al, az, dir, dif, 10000)


        saveComparisonImage(error, i, n_im, p_im, diff, type, mse, dir, dif, al, az, sc_p, sc_t, sc_diff,
                            solid_anlge_weighted_mse, solid_angle_weighted_error, gradient, 3000)
        saveComparisonImage(error, i, n_im, p_im, diff, type, mse, dir, dif, al, az, sc_p, sc_t, sc_diff,
                            solid_anlge_weighted_mse, solid_angle_weighted_error, gradient, 10000)
        ...
        p, t, t_mean, t_max = save_im_falsecolor(p_im, n_im, name, 3000, "results/results_3000/", IMAGE_SIZE[0], IMAGE_SIZE[1], IMAGE_SIZE[1])
        dgps_p.extend(p)
        dgps_t.extend(t)
        ...
        save_im_falsecolor(p_im, n_im, name, 10000, "results/results_10000/", IMAGE_SIZE[0], IMAGE_SIZE[1], IMAGE_SIZE[1])
...
def printStatistics(truth, prediction, file_name):
    ...
    with open("results/" + file_name, "w") as myfile:
        myfile.write("Test set statistic is: MSE %.5e, RMSE %.2f, LOG10_MSE %.5e, LOG10_RER %.5e"
                     "Relative Average Error Rate %.3f, "
                     "test solid angle MSE %.5e, test_Solid_Angle_RER %.3f, "
                     "test sc %.5e, test_gradient %.5e,"
                     % (get_mse(truth, prediction), get_rmse(truth, prediction), get_log10_mse(truth, prediction),
                        get_log10_relative_error(truth, prediction), get_relative_error(truth, prediction),
                        get_solid_angle_mse(truth, prediction), get_solid_angle_rer(truth, prediction),
                        get_spatial_contrast_MAE(truth, prediction),get_gradient_MAE(truth, prediction)
                        ))
        myfile.close()

def postProcessData(output_dim, log_reverse, n_ims, p_ims, gamma, log_base):
    num_ims = n_ims.shape[0]
    im_frame_n = np.zeros((num_ims, IMAGE_SIZE[0], IMAGE_SIZE[1], 1), dtype='float32')
    im_frame_p = np.zeros((num_ims, IMAGE_SIZE[0], IMAGE_SIZE[1], 1), dtype='float32')
    for i in range(num_ims):
        n_im = n_ims[i, :, :]
        p_im = p_ims[i, :, :]
        diff, n_im, p_im = postProcessIms(output_dim, n_im, p_im, "test_", gamma)
        if log_reverse:
            diff, n_im, p_im = post_process_log(n_im, p_im, log_base)
...
def train(retrain, loss_function, gamma, log_base, log_process, input_dim, output_dim, train_model, learning_rate):
    # start record time
    ...
    # reduce the learning rate by 0.5, if loss is not decreasing after each epoch until reach the minimum learning rate
    ...
    def loss_update(alpha, beta):
        def loss(y_true, y_pred):
            ...
            b = K.get_value(beta)
            print("b:" + str(beta))
            combo_loss = a*mse_rer_loss_old(y_true, y_pred) + b*mse_rer_loss_handle_log(y_true, y_pred)
            return combo_loss
        return loss

    model.compile(loss=loss_function, optimizer=keras.optimizers.adam(lr=learning_rate),
                  metrics=[metrics.mse, metrics.mae, relative_err, mse_log])

    batch_size = INITIAL_BATCH_SIZE
    min_batch = batch_size - 1
    if BATCH_REDUCTIONS > 0:
        batch_reductions = 2 ** (int(BATCH_REDUCTIONS))
        min_batch = np.max([MINI_BATCHSIZE, int(INITIAL_BATCH_SIZE / BATCH_REDUCTIONS)])
    loss = 1e10
    history = None
    if (os.path.exists("results/model/") == False):
        os.mkdir("results/model/")
    ...
    while batch_size > min_batch and loss > TARGET_LOSS:
        pb = printbatch()
        if train_model == "CNN":
            train_generator = trainGenerator_CNN(x_total, x_sun_total, y_total, batch_size)
            val_generator = trainGenerator_CNN(x_val, x_sun_val, y_val, batch_size)
            val_test_generator = testGenerator_CNN(x_val, x_sun_val, 1)
            test_generator = testGenerator_CNN(x_test, x_sun_test, 1)
        else:
            train_generator = trainGenerator_Dense(input_dim, x_total, x_sun_total, y_total, batch_size)
            val_generator = trainGenerator_Dense(input_dim, x_val, x_sun_val, y_val, batch_size)
            val_test_generator = testGenerator_Dense(input_dim, x_val, x_sun_val, 1)
            test_generator = testGenerator_Dense(input_dim, x_test, x_sun_test, 1)


        print("Built the generator")
        print(x_total.shape)
        history = model.fit_generator(train_generator, steps_per_epoch = int(x_total.shape[0]/batch_size), epochs = NUM_EPOCHS, verbose=2,
                        validation_data=val_generator, validation_steps=int(x_val.shape[0]),
                            #callbacks=[AttentionLoss(alpha, beta), pb,
                            callbacks=[pb,
                                ReduceLROnPlateau(monitor='loss', factor=0.5, patience=2, min_lr=MIN_LR, verbose=1),
                                ModelCheckpoint(filepath='results/model/weights.hdf5', verbose=1, save_best_only=True)])
    ...
    p_im_val = model.predict_generator(val_test_generator, steps=y_val.shape[0])
    print(p_im_val.shape)
    p_im_val = p_im_val.reshape(y_val.shape[0],IMAGE_SIZE[0], IMAGE_SIZE[1], 1)

    p_im_test = model.predict_generator(test_generator, steps=y_test.shape[0])
    print(p_im_test.shape)
    p_im_test = p_im_test.reshape(y_test.shape[0], IMAGE_SIZE[0], IMAGE_SIZE[1], 1)
    print(p_im_test.max())
    ...
    #plotAnalysis(output_dim, log_process, p_im_val, y_val, x_val, "test_", gamma, log_base)
    plotAnalysis(output_dim, log_process, p_im_test, y_test, x_test, "test_", gamma, log_base)

    y_test_absolute, p_im_test_absolute = postProcessData(output_dim, log_process, y_test, p_im_test, gamma, log_base)
    print(p_im_test.max())
    printStatistics(y_test_absolute, p_im_test_absolute, "absolute_statistics.txt")
...
RETRAIN = False
LOSS_FUNCTION_TYPE = mse_rer_loss_old
LOG_BOOL = True
SKYMAP_BOOL = False
MODEL_TYPE = "Dense"
GAMMA_VALUE = 2.2 #2.2
...
LONGITUDE = 122
SM = 120
AB4_DIR = "D:/Image_Based_Relighting/ALL_DATA/AB4_ANNUAL_PANORAMA_npy/"
AB0_DIR = "D:/Image_Based_Relighting/ALL_DATA/AB0_ANNUAL_PANORAMA_npy/"
SKY_DIR = "D:/Image_Based_Relighting/2018-11-30_genSkyImagesPanoramas_new_view_position/npy/"


genData.preprocess(log = LOG_BOOL, gamma = GAMMA_VALUE, skymap = SKYMAP_BOOL, log_base = LOG_BASE_VALUE,
                   num_clusters=NUM_CLUSTERS, gen_trainSet= False, latitude=LATITUDE, longitude=LONGITUDE,
                   sm=SM, ab4_dir = AB4_DIR, ab0_dir = AB0_DIR, sky_dir = SKY_DIR)
INPUT_DIM = 13
if SKYMAP_BOOL:
    INPUT_DIM = 14
LR = 0.0001
if MODEL_TYPE == "CNN":
    LR = 0.001
train(retrain=RETRAIN, loss_function = LOSS_FUNCTION_TYPE, gamma = GAMMA_VALUE, log_base = LOG_BASE_VALUE, log_process = LOG_BOOL, input_dim = INPUT_DIM, output_dim=1, train_model=MODEL_TYPE, learning_rate=LR)
...
diff, t, p = plot.postProcessIms(t, p, "test_", t.shape[0], 2.2, process_data_path)
